...
class TaskSuspendRequest:
	'''SOAP request to suspend a task on the controller.'''
	def __init__(self, taskName: str, createdBy: str, _internal = 0):
		...
		'''
		if(_internal == 0):
			self._instance = task_suspend_request(taskName, createdBy)
		else:
			self._instance = _internal

	@property
	def task_name(self) -> str:
		'''Name of the task to suspend.'''
	...
	def __eq__(self, other) -> bool:
		if not isinstance(other, TaskSuspendRequest):
			NotImplemented
